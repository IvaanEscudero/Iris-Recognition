# Archivo encargado de procesar y registrar masivamente imágenes de iris en paralelo dentro de la base de datos TileDB.
...
def MostrarEncabezado(hilos, ruta_destino=RUTA_BD, ruta_origen=RUTA_INPUT):
    print(f"--- INICIANDO REGISTRO MASIVO BBDD CASIA (PARALELO) ---")
...
def ProcesarImagenWorker(args):
    ...
    try:
        ...
        # --- SOLUCIÓN: Igualar dimensiones geométricas ---
        # Si la máscara tiene 240 columnas y la plantilla 480, duplicamos los bits de la máscara
...
def PrepararBaseDeDatosTileDB(ruta_destino=RUTA_BD, res_angular=RES_ANGULAR, res_radial=RES_RADIAL):
    if not path.exists(ruta_destino) or object_type(ruta_destino) != "array":
        ...
        # --- SOLUCIÓN: Orden correcto de dimensiones y columnas multiplicadas x2 ---
...
def RealizarRegistroParalelo(hilos, ruta_origen=RUTA_INPUT, ruta_destino=RUTA_BD, callback=None):
    ...
    carpetas_sujetos = [d.name for d in scandir(ruta_origen) if d.is_dir()]

    for sujeto in sorted(carpetas_sujetos):
        ruta_sujeto = path.join(ruta_origen, sujeto)
        for ojo in ('L', 'R'):
            ruta_ojo = path.join(ruta_sujeto, ojo)
            if not path.exists(ruta_ojo) or not path.isdir(ruta_ojo):
                continue
            nombre_usuario = f"{sujeto}_{ojo}"
            for archivo in scandir(ruta_ojo):
                if archivo.is_file() and archivo.name.lower().endswith(('.jpg','.png')):
            ...
            if exito:
                ...
                if len(batch_templates) >= batch_size:
                    ...
                    try:
                        ...
                        msg=f"Imagen {registrados_exito} de {total_imagenes} en curso."
...
def main(modo_debug):
    ...
    if modo_debug:
        print("\n[!] MODO DEBUG ACTIVADO")
        carpeta_temporal = mkdtemp(prefix="debug_casia_")
...
if __name__ == '__main__':
    parser = ArgumentParser(description="Script de registro masivo BBDD CASIA en paralelo optimizado para TileDB.")
