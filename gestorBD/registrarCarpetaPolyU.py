# Archivo encargado de procesar y registrar masivamente imágenes de iris en paralelo dentro de la base de datos TileDB (Estructura PolyU Simplificada).
...
def MostrarEncabezado(hilos, ruta_destino=RUTA_BD, ruta_origen=RUTA_INPUT):
    print(f"--- INICIANDO REGISTRO MASIVO BBDD PolyU (PARALELO) ---")
...
def RealizarRegistroParalelo(hilos, ruta_origen=RUTA_INPUT, ruta_destino=RUTA_BD, callback=None):
    ...
    # --- CAMBIO DE LÓGICA DE NAVEGACIÓN (PolyU Simplificada) ---
    # 1. Buscar los sujetos directamente en la carpeta raíz (ej. /001, /002...)
    carpetas_sujetos = [d for d in scandir(ruta_origen) if d.is_dir()]
    
    for sujeto in sorted(carpetas_sujetos, key=lambda d: d.name):
        
        # 2. Entrar a los ojos (L, R)
        for ojo in ('L', 'R'):
            ruta_ojo = path.join(sujeto.path, ojo)
            if not path.exists(ruta_ojo) or not path.isdir(ruta_ojo):
                continue
            
            # 3. Entrar DIRECTAMENTE a NIR (Ignoramos la carpeta VIS si existe)
            ruta_nir = path.join(ruta_ojo, "NIR")
            if not path.exists(ruta_nir) or not path.isdir(ruta_nir):
                continue
            
            # El ID en la base de datos será Sujeto_Ojo (ej. "001_L")
            nombre_usuario = f"{sujeto.name}_{ojo}"
            
            # 4. Extraer las imágenes (ej. 001_L_NIR_01.tiff)
            for archivo in scandir(ruta_nir):
                if archivo.is_file() and archivo.name.lower().endswith(('.jpg','.png', '.bmp', '.jpeg', '.tiff', '.tif')):
                    tareas.append((archivo.path, nombre_usuario, archivo.name))
    # -----------------------------------------------

    total_imagenes = len(tareas)
    if total_imagenes == 0:
        print("No se encontraron imágenes válidas NIR en la estructura de PolyU.")
        return registrados_exito, errores
...
def main(modo_debug):
    ...
    if modo_debug:
        print("\n[!] MODO DEBUG ACTIVADO")
        carpeta_temporal = mkdtemp(prefix="debug_polyu_")
        print(f"[!] Archivos se guardarán temporalmente en: {carpeta_temporal}\n")
        
        MostrarEncabezado(hilos_disponibles, carpeta_temporal)
        
        try:
            exitos, errores = MedirTiempo(RealizarRegistroParalelo, hilos_disponibles, carpeta_temporal)
        ...
        MostrarEncabezado(hilos_disponibles, RUTA_BD)
        exitos, errores = RealizarRegistroParalelo(hilos_disponibles, RUTA_BD)

    MostrarResumen(exitos, errores)

if __name__ == '__main__':
    parser = ArgumentParser(description="Script de registro masivo BBDD PolyU en paralelo optimizado para TileDB.")
