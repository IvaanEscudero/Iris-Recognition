# Archivo encargado de procesar y registrar masivamente imágenes de iris en paralelo dentro de la base de datos TileDB (Estructura UBIRIS).
...
def MostrarEncabezado(hilos, ruta_destino=RUTA_BD, ruta_origen=RUTA_INPUT):
    print(f"--- INICIANDO REGISTRO MASIVO BBDD UBIRIS (PARALELO) ---")
...
def RealizarRegistroParalelo(hilos, ruta_origen=RUTA_INPUT, ruta_destino=RUTA_BD, callback=None):
    ...
    # --- CAMBIO DE LÓGICA DE NAVEGACIÓN (UBIRIS) ---
    # 1. Buscar las carpetas de sesión (Sessao_1, Sessao_2...)
    carpetas_sessao = [d.name for d in scandir(ruta_origen) if d.is_dir() and d.name.lower().startswith("sessao")]

    for sessao in sorted(carpetas_sessao):
        ruta_sessao = path.join(ruta_origen, sessao)
        
        # 2. Buscar los sujetos dentro de la sesión (1, 2, 3...)
        carpetas_sujetos = [d.name for d in scandir(ruta_sessao) if d.is_dir()]
        
        for sujeto in sorted(carpetas_sujetos):
            ruta_sujeto = path.join(ruta_sessao, sujeto)
            
            # El ID en la base de datos será directamente el número de sujeto
            nombre_usuario = f"{sujeto}"
            
            # 3. Extraer las imágenes (Img_6_1_3.jpg)
            for archivo in scandir(ruta_sujeto):
    ...
    if total_imagenes == 0:
        print("No se encontraron imágenes válidas para procesar en la estructura de UBIRIS.")
        return registrados_exito, errores
...
def main(modo_debug):
    ...
    if modo_debug:
        print("\n[!] MODO DEBUG ACTIVADO")
        carpeta_temporal = mkdtemp(prefix="debug_ubiris_")
...
if __name__ == '__main__':
    parser = ArgumentParser(description="Script de registro masivo BBDD UBIRIS en paralelo optimizado para TileDB.")
