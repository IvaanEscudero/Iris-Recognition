# Realiza una prueba de aciertos y errores para un conjunto de imagenes (num_muestras) aleatorias (Estructura CASIA-Twins + Multiprocesamiento).
...
def ObtenerImagenesEstadisticas(ruta=RUTA_INPUT):
    """
    DESCRIPCIÓN:
        Recorre las carpetas buscando imágenes de la base de datos Twins.
        Estructura: Par_Gemelos -> [1L, 1R, 2L, 2R] -> Imagen.
    OUTPUT:
    ...
    # Recorrer las carpetas de los pares de gemelos (ej. /01, /02...)
    for par_gemelos in scandir(ruta):
        if par_gemelos.is_dir():
            # Recorrer las carpetas de los gemelos y sus ojos: Gemelo 1 (1L, 1R), Gemelo 2 (2L, 2R)
            for ojo_gemelo in ('1L', '1R', '2L', '2R'):
                ruta_ojo = path.join(par_gemelos.path, ojo_gemelo)
                
                if not path.exists(ruta_ojo) or not path.isdir(ruta_ojo):
                    continue
                    
                # El ID real es la combinación del par y el gemelo/ojo (ej. "01_1L")
                id_real = f"{par_gemelos.name}_{ojo_gemelo}"
...
def MostrarEstadisticas(stats, total):
    print("\n" + "="*55)
    print("Estadísticas de Precisión (Benchmark CASIA-Twins)")
    print("="*55)
...
def GenerarGraficoEstadisticas(stats, total, umbral):
    ...
    ax.set_title(f'Rendimiento del Sistema Biométrico (CASIA-Twins)\nMuestras evaluadas: {total} | Umbral de Hamming: {umbral}', 
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Evaluar la tasa de aciertos y rechazos del sistema de reconocimiento de iris (CASIA-Twins).")
