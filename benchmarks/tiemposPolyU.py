# Realiza una prueba de rendimiento de todas las fases del reconocimiento de iris (Estructura PolyU Simplificada)
...
def ObtenerImagenes(ruta=RUTA_INPUT):
    """
    DESCRIPCIÓN:
        Recorre buscando imágenes con la nueva estructura de PolyU
        (Raíz -> Sujeto -> L/R -> NIR).
    INPUT:
        ruta: Directorio raíz de PolyU.
    OUTPUT:
        todas_imagenes - Lista con las rutas absolutas de todas las imagenes NIR
    ...
    for sujeto in sorted(carpetas_sujetos, key=lambda d: d.name):
        for ojo in ('L', 'R'):
            ...
            # Entramos directamente a NIR, ignorando VIS
    ...
    if not todas_imagenes:
        print("No se encontraron imágenes NIR en la estructura de directorios.")
        return None
...
def GenerarGraficoTiempos(tiempos, exitos):
    ...
    ax.set_title(f'Rendimiento del Sistema: Distribución de Tiempos (PolyU)\nIteraciones: {exitos} | Tiempo Medio Total: {tiempo_total_medio:.4f}s', 
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Evaluar el tiempo de ejecución del sistema de reconocimiento de iris (PolyU).")
