# Realiza una prueba de aciertos y errores para un conjunto de imagenes (num_muestras) aleatorias (Estructura CASIA + Multiprocesamiento).
...
def ObtenerImagenesEstadisticas(ruta=RUTA_INPUT):
    ...
    for usuario in scandir(ruta):
        if usuario.is_dir():
            # Recorrer las carpetas de los ojos Izquierdo y Derecho
            for ojo in ('L', 'R'):
...
def GenerarGraficoEstadisticas(stats, total, umbral):
    ...
    ax.set_title(f'Rendimiento del Sistema Biométrico (CASIA)\nMuestras evaluadas: {total} | Umbral de Hamming: {umbral}', 
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Evaluar la tasa de aciertos y rechazos del sistema de reconocimiento de iris (CASIA Multiproceso).")
