# Realiza una prueba de aciertos y errores para un conjunto de imagenes (num_muestras) aleatorias (Estructura PolyU simplificada + Multiprocesamiento).
...
def ObtenerImagenesEstadisticas(ruta=RUTA_INPUT):
    """
    DESCRIPCIÓN:
        Recorre las carpetas buscando imágenes con la nueva estructura de PolyU
        (Raíz -> Sujeto -> L/R -> NIR).
    OUTPUT:
    ...
    # 1. Buscar los sujetos en la carpeta raíz (ej. /001, /002...)
    try:
        carpetas_sujetos = [d for d in scandir(ruta) if d.is_dir()]
    ...
    for sujeto in sorted(carpetas_sujetos, key=lambda d: d.name):
        ...
        for ojo in ('L', 'R'):
            ...
            # El ID real es Sujeto_Ojo (ej. "001_L") para que coincida con TileDB
            id_real = f"{sujeto.name}_{ojo}"
            ...
            try:
                for archivo in scandir(ruta_nir):
                    if archivo.is_file() and archivo.name.lower().endswith(('.jpg', '.png', '.bmp', '.jpeg', '.tiff', '.tif')):
                        todas_imagenes.append((archivo.path, id_real))
            except FileNotFoundError:
                pass
...
def ObtenerEstadisticas(todas_imagenes, num_muestras, umbral, callback=None):
    ...
    # imap_unordered devuelve las imagenes según terminan de procesarse
    for resultado in pool.imap_unordered(WorkerExtractorEstadisticas, rutas_a_procesar):
        ...
        # Comparación (Secuencial para proteger la RAM y TileDB)
        try:
            ...
            for match_id, _ in resultados:
                # Omitir el auto-emparejamiento con exactamente la misma imagen física
    ...
    pool.close()
    pool.join()
    print() # Salto de línea limpio tras el progreso
    return stats

def MostrarEstadisticas(stats, total):
    print("\n" + "="*55)
    print("Estadísticas de Precisión (Benchmark PolyU)")
    print("="*55)
...
def GenerarGraficoEstadisticas(stats, total, umbral):
    ...
    ax.set_title(f'Rendimiento del Sistema Biométrico (PolyU)\nMuestras evaluadas: {total} | Umbral de Hamming: {umbral}', 
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Evaluar la tasa de aciertos y rechazos del sistema de reconocimiento de iris (PolyU Multiproceso).")
