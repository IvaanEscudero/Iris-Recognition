# Realiza una prueba de rendimiento de todas las fases del reconocimiento de iris (Estructura UBIRIS)
...
def ObtenerImagenes(ruta=RUTA_INPUT):
    """
    DESCRIPCIÓN:
        Recorre buscando imágenes con la estructura de UBIRIS
        (Raíz -> Sessao_X -> Sujeto -> Imagen).
    INPUT:
        ruta: Directorio raíz de UBIRIS.
    OUTPUT:
    ...
    # Buscar las carpetas de las sesiones (ej. Sessao_1)
    try:
        carpetas_sessao = [d for d in scandir(ruta) if d.is_dir() and "sess" in d.name.lower()]
    except FileNotFoundError:
        return None

    for sessao in sorted(carpetas_sessao, key=lambda d: d.name):
        try:
            # Buscar las carpetas de los sujetos (ej. 1, 2, 3...)
        ...
        for sujeto in sorted(carpetas_sujetos, key=lambda d: d.name):
            try:
                for archivo in scandir(sujeto.path):
    ...
    if not todas_imagenes:
        print("No se encontraron imágenes en la estructura de directorios de UBIRIS.")
        return None
...
def MedicionTiempos(todas_imagenes, num_iteraciones, callback=None):
    ...
    for i in range(num_iteraciones):
        ...
        # cv2 - Leemos la imagen, forzando escala de grises con el '0'
...
def GenerarGraficoTiempos(tiempos, exitos):
    ...
    ax.set_title(f'Rendimiento del Sistema: Distribución de Tiempos (UBIRIS)\nIteraciones: {exitos} | Tiempo Medio Total: {tiempo_total_medio:.4f}s', 
...
def main(num_iteraciones):
    ...
    print(f"Evaluación con {num_iteraciones} iteraciones (UBIRIS).")
...
if __name__ == "__main__":
    parser = ArgumentParser(description="Evaluar el tiempo de ejecución del sistema de reconocimiento de iris (UBIRIS).")
